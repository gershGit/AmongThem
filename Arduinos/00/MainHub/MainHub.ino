...
{
    if (digitalRead(MEETING_BUTTON) == HIGH) {
        setLightsRFID_Ready();
    }
}
...
{
    //Swipe start
    if (digitalRead(LOG_SW) == HIGH && log_status == false) {
        state_begin_time = millis();
        clearLights();
        task_complete = false;
        log_status = true;
        log_start = millis();
        digitalWrite(GREEN_LED, HIGH);
    } else if (digitalRead(LOG_SW) == LOW && log_status == true) { //Swipe end
...
void checkAtomicClock()
{
    ...
    if (analogRead(ATOMIC_SW) < 800 && atomic_status == false && atomic_button_state == false) { //Start task
        Serial.println("Starting Atomic");
        task_active = 3;
        state_begin_time = millis();
